...
{
	VulkanContext::VulkanLibrary::VulkanLibrary()
		:
#if defined(_WIN32)
	...
	void VulkanContext::AddDebugMessenger(DebugCallback* debugCallback)
	{
		m_Instance->AddDebugMessenger(*debugCallback);
	}

	void VulkanContext::RemoveDebugMessenger(DebugCallback* debugCallback)
	{
		m_Instance->RemoveDebugMessenger(*debugCallback);
	}

	const uint32_t VulkanContext::GetFramesInFlight() const
	{
		return m_FramesInFlight;
	}

	const InstanceLoader& VulkanContext::GetInstanceLoader() const
	{
		return m_Instance->GetLoader();
	}

	const DeviceLoader& VulkanContext::GetDeviceLoader() const
	{
		return m_Device->GetLoader();
	}

	const VulkanContext::VulkanLibrary& VulkanContext::GetVulkanLibrary() const
	{
	...
	void VulkanContext::CreateCommandPool(uint32_t graphicsFamilyIndex)
	{
